...
function toggleEdit() {
    const isEditing = document.getElementById('edit-btn').textContent === 'Cancel';
    document.getElementById('edit-btn').textContent = isEditing ? 'Edit' : 'Cancel';

    document.querySelectorAll('input').forEach(input => (input.disabled = isEditing));
    document.querySelector('.save-btn').disabled = isEditing;

    if (isEditing) {
        showNotification('Editing canceled. Changes will not be saved.');
        resetForm();
        hideOtpContainers();
    } else {
        showNotification('You can now edit the fields.');
    }
}
function saveChanges() {
...
function handleEmailChange() {
    ...
    if (email !== originalEmail && validateEmail(email)) {
        ...
        document.getElementById('send-email-otp').disabled = false;
    } else {
...
function handleMobileChange() {
    ...
    const mobileContainer = document.getElementById('mob-otp-container');
    if (mobile !== originalMobile && validateMobile(mobile)) {
        mobileContainer.style.display = 'block';
        document.getElementById('send-mob-otp').disabled = false;
    } else {
...
function requestEmailOtp() {
    document.getElementById('verify-email-otp').disabled = false;
    document.getElementById('resend-email-otp').disabled = true;
    startOtpTimer('email');
    showNotification('Email OTP sent!');
}

function requestMobileOtp() {
    document.getElementById('verify-mob-otp').disabled = false;
    document.getElementById('resend-mob-otp').disabled = true;
    startOtpTimer('mob');
    showNotification('Mobile OTP sent!');
}

function verifyEmailOtp() {
    const otp = document.getElementById('email-otp').value;
    if (validateOtp(otp)) {
        showNotification('Email OTP verified!');
        saveEmailChange();
        hideOtpContainer('email');
    } else {
...
function verifyMobileOtp() {
    const otp = document.getElementById('mob-otp').value;
    if (validateOtp(otp)) {
        showNotification('Mobile OTP verified!');
        saveMobileChange();
        hideOtpContainer('mob');
    } else {
...
function startOtpTimer(type) {
    ...
    const resendButton = document.getElementById(`resend-${type}-otp`);
...
function saveMobileChange() {
    originalMobile = document.getElementById('mob').value;
    showNotification('Mobile updated successfully!');
}

function hideOtpContainer(type) {
    document.getElementById(`${type}-otp-container`).style.display = 'none';
}

function hideOtpContainers() {
    hideOtpContainer('email');
    hideOtpContainer('mob');
}
...
// Disable verify and resend buttons on page load
document.getElementById('verify-email-otp').disabled = true;
document.getElementById('resend-email-otp').disabled = true;
document.getElementById('verify-mob-otp').disabled = true;
document.getElementById('resend-mob-otp').disabled = true;
...
function validateForm() {
    ...
    // Show success notification or showNotification message after successful password change
